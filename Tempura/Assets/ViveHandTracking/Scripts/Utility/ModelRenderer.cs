...
public class ModelRenderer : MonoBehaviour {
  ...
  [SerializeField] public static bool _isInTheOil = false;
  private bool _isEnd = false;

  private GameObject _wristTracker;
  ...
  IEnumerator Start() {
    ...
    _wristTracker = GameObject.Find("PositionWrist");
  }

  void Update() {
    transform.localPosition = _wristTracker.transform.position;

    GestureResult result = IsLeft ? GestureProvider.LeftHand : GestureProvider.RightHand;
    if (_isEnd) {
      //Debug.Log("isEND");
      //Hand.SetActive(false);
      //GetComponent<>.enabled = True; //空白内トラッカーベースのトラッキングのスクリプトを挿入する
      return;
    }
    Hand.SetActive(true);
